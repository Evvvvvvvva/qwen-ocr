...
def run_cdm(input_path: str, output_root: str, pool_num: int = 240):
    """
    Run the full CDM pipeline: data preparation, bbox extraction and evaluation.
    :param input_path: Path to input JSON/JSONL file containing gt/pred lists.
    :param output_root: Directory where outputs will be written.
    :param pool_num: Number of parallel workers to use.
    """
    print(f"Running pipeline with input={input_path}, output={output_root}, pools={pool_num}")
    
    temp_dir = os.path.join(output_root, "temp_dir")
    exp_name = os.path.basename(input_path).split('.')[0]
    input_data = load_results(input_path)
    img_ids = []
    groundtruths = []
    predictions = []
    ...
    data_root = os.path.join(output_root, user_id)
    output_dir_info = {}
    input_args = []
    ...
    c = time.time()
    metrics_res, metric_res_path, match_vis_dir = evaluation(output_root, exp_name)
    d = time.time()
    ...
    return metrics_res['mean_score'], metrics_res['exp_rate']



if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Run LaTeX bbox extraction and evaluation pipeline.")
    parser.add_argument('--input', '-i', type=str, default="assets/example/input_example.json")
    parser.add_argument('--output', '-o', type=str, default="cdm-output")
    parser.add_argument('--pools', '-p', type=int, default=240)
    args = parser.parse_args()
    run_cdm(args.input, args.output, args.pools)
