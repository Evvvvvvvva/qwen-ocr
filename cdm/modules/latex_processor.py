...
def normalize_latex(l, rm_trail=False):
    ...
    # TODO when \operatorname * meets textcolor it comes error, yet the * is useless, so we simply remove it bynow.
...
def token_add_color(l_split, idx, render_dict):
    ...
    if token in PHANTOM_Tokens:
        ...
        # ** nomal token
        l_split_copy = l_split.copy()
        # TODO sometimes there is translation after add color, the exp prove that \textcolor{black}{ A } is better than \textcolor{black}{A}
        l_split_copy[idx] = r'\textcolor{black}{ ' + l_split_copy[idx] + ' }'
...
def token_add_color_RGB(l_split, idx, token_list, brace_color=False):
    """using \textcolor[RGB]{r,g,b} to render latex. 
    """
    token = l_split[idx]
    if not token:
        next_idx = idx + 1
