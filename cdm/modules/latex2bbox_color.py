...
from cdm.modules.latex_processor import (
    normalize_latex,
    token_add_color_RGB
)
...
def convert_pdf2img(pdf_filename, png_filename):
    cmd = "gs -dSAFER -dBATCH -dNOPAUSE -sDEVICE=png16m -r300 -sOutputFile=%s %s >/dev/null"%(png_filename, pdf_filename)
    os.system(cmd)
...
def latex2bbox_color(input_arg):
    ...
    try:
        ...
        if len(token_list) > 1300:
            ...
            paper_size = 5
        latex_template = formular_template.replace("<PaperSize>", str(paper_size))
        final_latex = latex_template.replace("<Formula>", str(rgb_latex))
